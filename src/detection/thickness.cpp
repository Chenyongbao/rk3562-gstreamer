...
FixedZSampleResult sampleDistanceAtFixedZ(double z_target_mm, double feedrate, const std::string& log_prefix) {
    ...
    if (!klipper_->sendGcode(z_script.str(), nullptr, kFixedSampleMoveTimeoutSec, &move_error)) {
        result.status = FixedZSampleStatus::MoveFailed;
        return result;
    }

    std::string trigger_error;
    if (!klipper_->sendGcode("LASER_RANGE_SENSOR SENSOR=my_range_sensor",
                                              nullptr,
    ...
    for (int attempt = 0; attempt < kFixedSampleQueryRetries; ++attempt) {
        ...
        if (klipper_->queryLaserDistance(distance_mm, valid, &query_error, 1)) {
...
ThicknessService::ThicknessService(KlipperManager* klipper, const ThicknessConfig& config)
    : feedrate_(config.feedrate),
      ...
      klipper_(klipper) {}
//面积检测逻辑
bool ThicknessService::selectTargetDetection(const std::vector<SegmentationResult>& yolo_results,
                                             SegmentationResult& selected_det,
    ...
    selected_det = yolo_results[0];

    if (yolo_results.size() == 1) {
    ...
    for (size_t i = 0; i < yolo_results.size(); ++i) {
        const SegmentationResult& det = yolo_results[i];
        const long long w = std::max(det.boundingBox.width, 0);
        const long long h = std::max(det.boundingBox.height, 0);
    ...
    selected_det = yolo_results[best_index];
...
bool ThicknessService::moveToXY(double x, double y, std::string& errorMsg) const {
    ...
    std::string move_error;
    if (!klipper_->sendGcode(script.str(), nullptr, 20L, &move_error)) {
...
bool ThicknessService::probeDistanceAndZDrop(double& out_distance_mm,
    ...
    auto forceHomeAfterNoData = [&](const std::string& reason) {
        ...
        if (!klipper_->forceHome(&home_error)) {
...
//拿到yolo检测结果，ai识别+机械控制
bool ThicknessService::measureFromYolo(const std::vector<SegmentationResult>& yolo_results,
                                       ThicknessResult& result,
                                       std::string& errorMsg) {
    // 统一初始化，避免调用方读取到脏数据
    result = ThicknessResult{};
    result.triggered = true;
    result.detection_count = static_cast<int>(yolo_results.size());
    ...
    try {
        do {
            if (!yolo_results.empty()) {
                ...
                SegmentationResult det{};
                bool selected_by_max_area = false;
                if (!selectTargetDetection(yolo_results, det, selected_by_max_area, errorMsg)) {
                    break;
                }
                result.selected_by_max_area = selected_by_max_area;
                result.class_id = det.classId;
                result.confidence = det.confidence;
                result.roi_x = det.boundingBox.x;
                result.roi_y = det.boundingBox.y;
                result.roi_w = det.boundingBox.width;
                result.roi_h = det.boundingBox.height;

                result.center_x = static_cast<double>(det.boundingBox.x) + static_cast<double>(det.boundingBox.width) / 2.0;
                result.center_y = static_cast<double>(det.boundingBox.y) + static_cast<double>(det.boundingBox.height) / 2.0;
