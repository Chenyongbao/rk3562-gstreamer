...
public:
    /**
     * @brief 构造函数
     * @param klipper Klipper 服务（依赖注入）
     * @param config 配置参数
     */
    explicit ThicknessService(KlipperManager* klipper = nullptr,
                              const ThicknessConfig& config = ThicknessConfig{});
     ...
     * @param yolo_results DETECT 命令本次推理结果
     * @param result 输出检测结果
     * @param errorMsg 错误信息
     * @return true 成功，false 失败
     */
    bool measureFromYolo(const std::vector<SegmentationResult>& yolo_results, ThicknessResult& result, std::string& errorMsg);

private:
    bool selectTargetDetection(const std::vector<SegmentationResult>& yolo_results,
