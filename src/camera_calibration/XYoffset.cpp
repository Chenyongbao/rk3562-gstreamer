...
XYoffsetService::XYoffsetService(KlipperManager* klipper,
...
bool XYoffsetService::captureImage(cv::Mat& out, std::string& errorMsg) const {
    ...
    // 取一帧比"当前最新"更新的帧（即刚采集的新帧），而非缓存旧帧。
    Snapshot snap;
    if (!frame_provider_->grabNewerThan(frame_provider_->latestFrameId(), snap,
                                        nullptr, kMainCameraRefreshTimeoutMs)) {
        errorMsg = "Failed to copy frame from main camera";
        return false;
    }
    if (snap.nv12.size() < frameSize) {
        errorMsg = "Frame too small: " + std::to_string(snap.nv12.size());
        return false;
    }

    uint8_t* data = snap.nv12.data();
    cv::Mat yPlane(height, width, CV_8UC1, data);
    cv::Mat uvPlane(height / 2, width / 2, CV_8UC2,
...
bool XYoffsetService::calibrate(double& outDxPx, double& outDyPx, std::string& errorMsg) {
    KlipperManager& klipper = *klipper_;
