...
static bool sendRequiredGcode(KlipperManager* klipper, const std::string& script, long timeout_sec, std::string& error);
...
static bool waitForFocusMode(KlipperManager* klipper,
                             const char* gcode,
    ...
    while (std::chrono::steady_clock::now() <= deadline) {
        ...
        if (gcode && *gcode && now >= next_retry && retry_count < max_retry_count) {
            ...
            if (!sendRequiredGcode(klipper, gcode, timeout_sec, resend_error)) {
...
static bool switchFocusMode(KlipperManager* klipper,
                            const char* gcode,
                            const char* expected_mode,
                            long timeout_sec,
                            int wait_timeout_ms,
    ...
    if (!sendRequiredGcode(klipper, gcode ? gcode : "", timeout_sec, error)) {
        return false;
    }
    return waitForFocusMode(klipper, gcode, expected_mode, timeout_sec, wait_timeout_ms, error);
}
...
static bool sendRequiredGcode(KlipperManager* klipper, const std::string& script, long timeout_sec, std::string& error) {
    ...
    if (!klipper->sendGcode(script, nullptr, timeout_sec, &local_error)) {
...
// fill interior holes (white core) via contour 
count filled pixels.
...
public:
    KlipperManager* klipper_ = nullptr;  // 依赖注入，供静态辅助函数通过 frames.klipper_ 访问
    explicit FocalFrameSource(KlipperManager* klipper, FrameProvider* frame_provider)
        : klipper_(klipper), frame_provider_(frame_provider) {}
    ...
    bool grabBgrFrame(cv::Mat& out_bgr, std::string& error, uint64_t* out_frame_id = nullptr) {
        if (!frame_provider_) {
            error = "FrameProvider not available";
            return false;
        }
        // 取一帧比"当前最新"更新的帧，而非缓存旧帧。
        Snapshot snap;
        if (!frame_provider_->grabNewerThan(frame_provider_->latestFrameId(), snap,
                                            nullptr, 5000)) {
            error = "Failed to copy latest main camera frame";
            return false;
        }
        if (snap.nv12.size() < main_nv12_.size()) {
...
static bool moveToZ(KlipperManager* klipper, double z_mm, double feedrate, std::string& error) {
    return sendRequiredGcode(klipper, formatMoveZScript(z_mm, feedrate), 40L, error);
}
static void logFocusStepperStatus(KlipperManager* klipper) {
    std::string response;
    std::string error;
    if (!klipper->sendGcode("stepper_status motor=1", &response, 20L, &error)) {
...
static bool initFocusStepperAndDetectMode(KlipperManager* klipper,
                                          FocalMachineType& out_machine_type,
    ...
    if (!sendRequiredGcode(klipper, "stepper_init", 20L, error)) {
        return false;
    }
...
static bool grabAtZ(FocalFrameSource& frames,
    ...
    if (!std::isfinite(last_z) || std::abs(last_z - z_mm) > 0.001) {
        if (!moveToZ(frames.klipper_, z_mm, config.z_feedrate, error)) {
            return false;
        }
...
static bool measureFocusDistance(FocalFrameSource& frames,
    ...
    if (!moveToZ(frames.klipper_, final_z, config.z_feedrate, move_error)) {
...
Focalabfocal::Focalabfocal(KlipperManager* klipper, FrameProvider* frame_provider)
    : config_(),
      klipper_(klipper),
      frame_provider_(frame_provider) {}

Focalabfocal::Focalabfocal(const FocalAutoFocusConfig& config,
                           KlipperManager* klipper,
...
bool Focalabfocal::runAutoFocus(FocalAutoFocusResult& result) {
    ...
    FocalFrameSource frames(klipper_, frame_provider_);
    if (!frames.init(result.error)) {
        return false;
    }

    std::string local_error;
    if (!klipper_->forceHome(&local_error)) {
        result.error = local_error.empty() ? "Focal homing failed" : local_error;
        return false;
    }

    sendRequiredGcode(klipper_, "SET_IDLE_TIMEOUT TIMEOUT=1800", 20L, local_error);
    if (!initFocusStepperAndDetectMode(klipper_,
                                       result.machine_type,
    ...
    if (!sendRequiredGcode(klipper_, formatMoveScript(config_.focus_x_mm,
                                            config_.focus_y_mm,
                                            config_.xy_feedrate),
                           40L,result.error)) {
        return false;
    }
    if (!klipper_->setFillLight(0, &result.error)) {
        return false;
    }
    if (!klipper_->sendGcode("M3 S1", nullptr, 20L, &result.error)) {
        return false;
    }
    ...
    for (double z : {10.0, 20.0, 25.0}) {
        if (!moveToZ(klipper_, z, config_.z_feedrate, result.error)) {
            return false;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(300));
    }
    ...
    logFocusStepperStatus(klipper_);
    frames.flushForMs(config_.stepper_flush_ms);
...
void Focalabfocal::cleanupHardware() {
    std::string ignored;
    klipper_->laserOff(&ignored);
    klipper_->setFillLight(128, &ignored);
    klipper_->forceHome(&ignored);
}
