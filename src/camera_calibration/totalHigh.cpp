...
TotalHighService::TotalHighService(KlipperManager* klipper,
                                   double feedrate, const char* confPath)
...
bool TotalHighService::measure(double& outHeight, std::string& errorMsg) {
    try {
        ...
        if (!klipper_->queryLaserDistance(
...
void TotalHighService::sendGcodeScript(const std::string& script) {
    std::string err;
    if (!klipper_->sendGcode(script, nullptr, 20L, &err)) {
