...
#define CALIB_LOCK pthread_mutex_lock(&CalibMutex);
#define CALIB_UNLOCK pthread_mutex_unlock(&CalibMutex);
#else
#define CALIB_LOCK
#define CALIB_UNLOCK
#endif

static int bCancel = 0;
...
static void icvGetQuadrangleHypotheses(const std::vector<std::vector< Point > > & contours, const std::vector< Vec4i > & hierarchy, std::vector<std::pair<float, int> >& quads, int class_id)
{
	const float min_aspect_ratio = 0.3f;
	const float max_aspect_ratio = 3.0f;
	const float min_box_size = 10.0f;
	typedef std::vector< std::vector< Point > >::const_iterator iter_t;
	iter_t i;
...
static bool checkQuads(vector<pair<float, int> > & quads, const Size & size)
{
...
static bool myCheckChessboard(InputArray _img, Size size)
{
	Mat img = _img.getMat();
                           ...
                           OutputArray corners_, int flags)
{
    ...
    if (flags & CALIB_CB_FAST_CHECK)
    {
        ...
        {
            if (!myCheckChessboard(img, pattern_size))
            {
                corners_.release();
                return false;
            }
    ...
    DPRINTF("Chessboard detection result 0: %d", found);
    ...
    DPRINTF("Chessboard detection result 1: %d", found);

    if (found)
        found = detector.checkBoardMonotony(out_corners);

    DPRINTF("Chessboard detection result 2: %d", found);
...
void ChessBoardDetector::generateQuads(const Mat& image_, int flags)
{
    ...
    int min_size = cvRound( image_.cols * image_.rows * .03 * 0.01 * 0.92 ); //25;
