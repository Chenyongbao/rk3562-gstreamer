...
static std::vector<Point2d> predict_corners(const Corner& corners,
                                         const std::vector<int>& p1,
                                         const std::vector<int>& p2,
                                         const std::vector<int>& p3) {
  std::vector<Point2d> pred(p3.size());
  if(p1.empty()) {
    ...
    for(int i = 0; i < pred.size(); ++i) {
      // compute vectors
      Point2d v1 = corners.p[p2[i]] - corners.p[p1[i]];
      Point2d v2 = corners.p[p3[i]] - corners.p[p2[i]];
...
static std::vector<int> predict_board_corners(const Corner& corners,
                                       std::vector<int>& used,
                                       std::vector<int>& p1,
                                       std::vector<int>& p2,
                                       std::vector<int>& p3) {
  std::vector<Point2d> pred = predict_corners(corners, p1, p2, p3);
  std::vector<int> pred_idx(pred.size(), -2);

  // build distance matrix
  std::vector<std::vector<double>> D(pred.size(), std::vector<double>(corners.p.size(), DBL_MAX));
  for(int i = 0; i < pred.size(); ++i) {
    Point2d w = pred[i] - corners.p[p3[i]];
    //    double angle_w = std::atan2(w.y, w.x);
    for(int j = 0; j < corners.p.size(); ++j) {
      if(used[j] == 1) {
        continue;
      }
      Point2d v_tmp = corners.p[j] - corners.p[p3[i]];
      Point2d v(v_tmp.dot(w), v_tmp.dot(Point2d(w.y, -w.x)));
      v         = v / (norm(w) * norm(w));
      double d1 = std::atan2(v.y, v.x);
      double d2 = (1 - norm(v));
      D[i][j]   = std::sqrt(std::abs(d1) + d2 * d2 * d2 * d2);
      //      Point2d v = corners.p[j] - corners.p[p3[i]];
      //      double d_angle = angle_w - std::atan2(v.y, v.x);
      //      double d1 = 1 - norm(v) / norm(w);
...
static bool add_board_boundary(Board& board, int direction) {
...
BOARD_GROW_TYPE grow_board(const Corner& corners, std::vector<int>& used, Board& board,
    ...
    case 0: {
      for(int i = rows - 4; i >= 0; --i) {
        for(int j = 0; j < cols; ++j) {
          ...
          proposal.emplace_back(Point2i(j, i));
        }
      }
  ...
  case 0: {
    for(int i = 0; i < cols; ++i) {
      ...
      p2.emplace_back(idx2);
      p3.emplace_back(idx3);
      proposal.emplace_back(Point2i(i, 0));
    }
    break;
  }
  case 1: {
    for(int i = 0; i < rows; ++i) {
      ...
      p2.emplace_back(idx2);
      p3.emplace_back(idx3);
      proposal.emplace_back(Point2i(0, i));
    }
    break;
  }
  case 2: {
    for(int i = 0; i < cols; ++i) {
      ...
      p2.emplace_back(idx2);
      p3.emplace_back(idx3);
      proposal.emplace_back(Point2i(i, rows - 1));
    }
    break;
  }
  case 3: {
    for(int i = 0; i < rows; ++i) {
      ...
      p2.emplace_back(idx2);
      p3.emplace_back(idx3);
      proposal.emplace_back(Point2i(cols - 1, i));
    }
    break;
  }
  default:
