...
//解析接收到的json数字
bool parseJsonNumber(const std::string& body, const char* key, double& out) {
...
bool KlipperManager::sendGcode(const std::string& script,
                               ...
                               std::string* err) {
    //执行gcode的字段
    const std::string payload = std::string("{\"script\":\"") + escapeJson(script) + "\"}";
...
bool KlipperManager::queryFocal(double& focal_long,
    ...
    if (!parseJsonNumber(body, "focal_long", focal_long)
        || !parseJsonNumber(body, "focal_short", focal_short)) {
